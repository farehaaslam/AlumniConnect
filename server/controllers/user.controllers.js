...
async function filterUsers(req,res){
  try {
    const { department, passOutYear, jobRole, city, state } = req.query;

    // Build dynamic query object
    let query = {};

    if (department) query.department = department;
    if (passOutYear) query.passOutYear = Number(passOutYear);
...
async function searchUsersByName(req,res){
  try {
    ...
    const users = await userModel.find({
      fullName: { $regex:  fullName, $options: 'i' }, // partial, case-insensitive match
    });
